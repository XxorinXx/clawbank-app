/**
 * Transaction Size Simulation for Agent Activation
 *
 * Tests whether the 4 Squads v4 instructions for agent activation
 * fit within Solana's 1232-byte versioned transaction limit.
 *
 * Instructions tested:
 *   1. configTransactionCreate  — AddMember(Initiate) + AddSpendingLimit
 *   2. proposalCreate
 *   3. proposalApprove
 *   4. configTransactionExecute (with spendingLimits PDA)
 *
 * Usage:  node scripts/test-activation-tx-size.mjs
 */
...
// Well-known USDC mint on mainnet (just used as a realistic pubkey)
const USDC_MINT = new PublicKey("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v");

// ── Helpers ────────────────────────────────────────────────────────
function buildActivationInstructions({ sponsor, agentPubkey, createKey, multisigPda, transactionIndex }) {
  const [spendingLimitPda] = multisig.getSpendingLimitPda({
    multisigPda,
    createKey: createKey.publicKey,
  });

  const initiateOnly = multisig.types.Permissions.fromPermissions([
    multisig.types.Permission.Initiate,
  ]);

  // 1. configTransactionCreate with two actions: AddMember + AddSpendingLimit
...
function main() {
  console.log("═══════════════════════════════════════════════════");
  console.log("  Agent Activation Transaction Size Simulation");
  console.log("═══════════════════════════════════════════════════");
  ...
  // ── Test 1: Base activation (4 Squads instructions) ──
  ...
  baseTx.sign([sponsor]);
  const baseResult = analyseTransaction("Base Activation (4 Squads IXs)", baseTx);
  ...
  if (baseResult.fits && budgetResult.fits) {
    console.log("\n  → All agent activation instructions fit in a single versioned transaction.");
